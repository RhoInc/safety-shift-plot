...
function syncControlInputs(controlInputs, settings) {
    ...
    if (Array.isArray(settings.filters) && settings.filters.length) settings.filters = settings.filters.map(function (filter) {
        var filterObject = {
...
function defineLayout(element) {
    ...
    container.append('div').classed('ssp-component', true).attr('id', 'ssp-controls');
    container.append('div').classed('ssp-component', true).attr('id', 'ssp-chart');
    container.append('div').classed('ssp-component', true).attr('id', 'ssp-listing');
}

function defineStyles() {
        var styles = ['#safety-shift-plot {' + '    width: 100%;' + '    display: inline-block;' + '}', '.ssp-component {' + '    margin: 0;' + '    border: none;' + '    padding: 0;' + '    display: inline-block;' + '}',

        //controls
        '#ssp-controls {' + '    width: 25%;' + '    float: left;' + '}', '#ssp-controls .control-group {' + '    width: 98%;' + '    margin: 0 2% 5px 0;' + '    padding: 0;' + '}', '#ssp-controls .control-group > * {' + '    display: inline-block;' + '}', '#ssp-controls .changer {' + '    float: right;' + '    width: 50%;' + '}', '#ssp-controls .wc-control-label {' + '    text-align: right;' + '    width: 48%;' + '}', '#ssp-controls .annote {' + '    width: 98%;' + '    text-align: right;' + '}',

        //chart
        '#ssp-chart {' + '    width: 36%;' + '    margin: 0 2%;' + '}',

        //listing
        '#ssp-listing {' + '    width: 35%;' + '    float: right;' + '}', '#ssp-listing .wc-table table {' + '    width: 100%;' + '    display: table;' + '}', '#ssp-listing .wc-table th:not(:first-child),' + '#ssp-listing .wc-table td:not(:first-child) {' + '    text-align: right;' + '}'];
...
function checkFilters() {
    ...
    if (this.config.filters) this.config.filters = this.config.filters.filter(function (filter) {
        var variableExists = _this.raw_data[0].hasOwnProperty(filter.value_col);
        var nLevels = d3.set(_this.raw_data.map(function (d) {
            return d[filter.value_col];
        })).values().length;

        if (!variableExists) console.warn(' The [ ' + filter.label + ' ] filter has been removed because the variable does not exist.');else if (nLevels < 2) console.warn('The [ ' + filter.label + ' ] filter has been removed because the variable has only one level.');
...
function getVisits() {
    ...
    this.visits = d3.set(this.initial_data.map(function (d) {
...
function addFilters(chart) {
    chart.config.filters.forEach(function (filter) {
        ...
        filter.values = d3.set(chart.initial_data.map(function (d) {
            return d[filter.value_col];
        })).values();
        filter.value = 'All';
        ...
        controlGroup.append('span').classed('wc-control-label', true).text(filter.label);
...
function onLayout() {
    ...
    this.wrap.append('p').attr('class', 'record-note').style('font-weight', 'bold').text('Click and drag to select points');
...
function updateParticipantCount(chart, selector, id_unit) {
    ...
    var totalObs = d3.set(chart.initial_data.map(function (d) {
...
function onResize() {
    ...
    //get list of visits
    var possibleVisits = d3.set(this.initial_data.filter(function (f) {
    ...
    function brushed() {
        ...
        if (selected_data.length === 0) this.listing.wrap.style('display', 'none');else this.listing.wrap.style('display', 'block');
...
function safetyShiftPlot(element, settings) {
    ...
    //controls
    var controls = webcharts.createControls(document.querySelector(element).querySelector('#ssp-controls'), {
        location: 'top',
        inputs: syncedControlInputs
    });

    //chart
    var chart = webcharts.createChart(document.querySelector(element).querySelector('#ssp-chart'), syncedSettings, controls);
    ...
    //listing
    var listing = webcharts.createTable(document.querySelector(element).querySelector('#ssp-listing'), listingSettings);
    listing.init([]);
