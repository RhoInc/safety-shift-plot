(function (global, factory) {
	...
	function onLayout() {
	    ...
	    this.config.filters.forEach(function (filter) {
	        ...
	        filter.values = d3.set(_this.super_raw_data.map(function (d) {
	            return d[filter.value_col];
	        })).values();
	        filter.value = 'All';

	        //Attach filter to the DOM.
	        var controlGroup = _this.controls.wrap.append('div').classed('control-group', true).datum(filter);
	        ...
	        changer.on('change', function (d) {
	            ...
	            _this.filteredData = _this.super_raw_data.filter(function (di) {
	                var filtered = false;
	                _this.config.filters.forEach(function (dii) {
	            ...
	            var nextRawData = preprocessData.call(_this, _this.filteredData);
